...
object Dependencies {
  ...
  // CDH 5.3 comes with akka 2.2.3-shaded-protobuf, which is built against com.typesafe.config-1.0.2,
  ...
  // CDH 5.3 comes with akka 2.2.3-shaded-protobuf preinstalled, so we should build against that version.
  val akkaVersion = "2.2.3-shaded-protobuf"
  
  // io.spray 1.2.3 pulls in a newer akka version than the one included in CDH 5.3. We neeed to downgrade
  ...
  // CDH 5.3 comes with spark 1.2.0-cdh5.3.6, hence we need to downgrade.
  // USED TO BE: 1.4.1
